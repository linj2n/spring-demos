...
public class UserService {
    ...
    public User createUserInformation(String password, String username, String email) {

        /* Users who have already ACTIVATED are not allowed to register */
        if (checkIfExitUserActivatedByLoginOrEmail(username,email)) {
            throw new UserAlreadyExistException("User has already existed. [username=" + username + ",email=" + email + "]");
        }

        User newUser = userRepository.findOneByUsernameOrEmail(username, email).filter(user -> !user.isActivated()).orElse(new User());
    ...
    public Boolean checkIfExitUserActivatedByLoginOrEmail(String login,String email) {
        User user = userRepository.findOneByUsernameOrEmail(login.toLowerCase(), email.toLowerCase()).orElse(null);
